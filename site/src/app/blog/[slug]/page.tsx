  ...
  return blocks.map((block, i) => {
    ...
    if (trimmed.startsWith("### ")) {
      return (
        <h3
          key={i}
          className="mb-4 mt-10 text-xl font-bold text-[#1a2332]"
        >
          {trimmed.replace(/^### /, "")}
        </h3>
      );
    ...
    if (trimmed.startsWith("## ")) {
      return (
        <h2
          key={i}
          className="mb-4 mt-12 text-2xl font-bold text-[#1a2332]"
        >
          {trimmed.replace(/^## /, "")}
        </h2>
      );
              ...
              <li key={j} className="flex items-start gap-3">
                ...
                <span
                  ...
                  dangerouslySetInnerHTML={{
                    __html: text.replace(
                      /\*\*(.+?)\*\*/g,
                      '<strong class="font-semibold text-[#1a2332]">$1</strong>'
                    ),
                  }}
                />
              </li>
...
export default async function BlogPostPage({ params }: PageProps) {
  ...
  return (
    <>
      {/* Article header */}
      <section className="bg-[#1a2332] py-20 sm:py-28">
        <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
          <span className="inline-block rounded-full bg-[#c9a96e]/20 px-4 py-1.5 text-sm font-medium text-[#c9a96e]">
            {post.category}
          </span>
      ...
      <section className="pb-16">
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
          <Link
            href="/blog"
            className="inline-flex items-center gap-2 text-sm font-semibold text-[#c9a96e] transition-colors hover:text-[#b8954f]"
          >
            <svg
